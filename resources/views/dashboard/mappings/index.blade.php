...
<div class="flex items-center justify-between mb-6">
    <div>
        <p class="text-sm text-gray-500 mt-0.5">
            Map {{ $erpDisplayName }} values to their equivalents in
            <span class="font-medium text-indigo-600">{{ $ecomDisplayName }}</span> /
...
<div x-show="showAdd" x-cloak x-transition
    ...
    <form method="POST" action="{{ route('dashboard.mappings.store', $type) }}" class="grid grid-cols-2 gap-3 md:grid-cols-5">
        ...
        <div>
            <label class="block text-xs font-medium text-gray-600 mb-1">{{ $erpDisplayName }} ID</label>
...
<div x-show="showImport" x-cloak x-transition
    ...
    <p class="text-xs text-amber-700 mb-3">Paste a JSON array. Each object needs <code>erp_id</code> (or <code>odoo_id</code>), <code>external_id</code>, and optionally <code>channel</code>, <code>odoo_label</code>, <code>external_label</code>.</p>
    <form method="POST" action="{{ route('dashboard.mappings.import', $type) }}">
        @csrf
        <textarea name="json_data" rows="5" placeholder='[{"erp_id":"5","erp_label":"WH/Stock","external_id":"69188747343","external_label":"Main Warehouse","channel":"shopify"}]'
    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="bg-gray-50 border-b border-gray-200">
                ...
                <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $erpDisplayName }} ID</th>
                <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $erpDisplayName }} Label</th>
            ...
            <tr class="hover:bg-gray-50 transition {{ $mapping->is_active ? '' : 'opacity-50' }}"
                ...
                <td class="px-4 py-3">
                    @if($mapping->channel === 'shopify')
                        <span class="badge bg-green-100 text-green-700">{{ $ecomDisplayName }}</span>
                ...
                {{-- ERP ID --}}
                <td class="px-4 py-3 font-mono text-xs text-gray-700">{{ $mapping->erp_id ?? $mapping->odoo_id }}</td>

                {{-- ERP Label --}}
                <td class="px-4 py-3 text-gray-800 font-medium">{{ $mapping->erp_label ?? $mapping->odoo_label ?: '—' }}</td>
