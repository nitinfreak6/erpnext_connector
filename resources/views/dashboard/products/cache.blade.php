...
<div x-data="{ selected: [], selectAll: false }"
     x-on:change="selectAll ? selected = {{ json_encode($products->pluck('odoo_id')->toArray()) }} : selected = []">
...
<div class="flex flex-wrap items-center justify-between gap-3 mb-4">
    ...
    <div class="flex flex-wrap items-center gap-2">
        ...
        {{-- Post to Shopify --}}
        <form method="POST" action="{{ route('dashboard.product-cache.post-shopify') }}" id="shopifyForm">
            @csrf
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50 transition">
                ...
                {{-- Odoo ID --}}
                <td class="px-4 py-3 font-mono text-xs text-gray-600">{{ $product->odoo_id }}</td>
                ...
                <td class="px-4 py-3">
                    <div class="flex flex-col gap-1">
                        ...
                        <span class="text-xs text-red-500 truncate max-w-24" title="{{ $product->shopify_message }}">
                ...
                <td class="px-4 py-3 text-right">
                    <div class="flex items-center justify-end gap-1">
                        {{-- View raw data --}}
                        <a href="{{ route('dashboard.product-cache.show', $product->odoo_id) }}"
                        ...
                        {{-- Refresh cache --}}
                        <form method="POST" action="{{ route('dashboard.product-cache.refresh', $product->odoo_id) }}">
                            @csrf
                            <button type="submit"
                        ...
                        {{-- Clear --}}
                        <form method="POST" action="{{ route('dashboard.product-cache.clear', $product->odoo_id) }}"
